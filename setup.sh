...
echo
echo "[2/10] Preparing package manager..."

# Remove any old/broken Ookla repository.
# Ookla's packagecloud repository currently returns 404 for
# Ubuntu Noble, so we do not add it to APT.
echo "[CHECK] Looking for old Ookla repositories..."

sudo find /etc/apt/sources.list.d \
    -type f \
    \( -iname '*ookla*' -o -iname '*speedtest*' \) \
...
# ------------------------------------------------------------
# Install Keyd BEFORE copying configuration
# ------------------------------------------------------------

install_package keyd

# ------------------------------------------------------------
# Exact configuration to migrate
# ------------------------------------------------------------
...
if command -v speedtest >/dev/null 2>&1; then
    ...
    # --------------------------------------------------------
    # Ookla's packagecloud repository currently does not
    # provide an Ubuntu Noble repository.
    #
    # Use the official Ookla Ubuntu Jammy amd64 package.
    # It installs /usr/bin/speedtest.
    # --------------------------------------------------------

    SPEEDTEST_URL="https://packagecloud.io/ookla/speedtest-cli/packages/ubuntu/jammy/speedtest_1.2.0.84-1.ea6b6773cf_amd64.deb/download.deb?distro_version_id=237"

    SPEEDTEST_DEB="/tmp/speedtest_ookla.deb"

    echo "[DOWNLOAD] Downloading Ookla Speedtest..."
